...
class BoggleSetUpTests(TestCase):
    ...
    def test_board(self):
        with app.test_client() as client:
            ...
            html = res.get_data(as_text=True)
            self.assertEqual(res.status_code, 200)
            self.assertIn('<th colspan="5" class="boggle-board text-center">Game #1</th>')
...
class BoggleGameplayTests(TestCase):
    ...
    def test_played_word_correct(self):
        with app.test_client() as client:
            res = client.get(f'/played-word?word={choide:new_correct_words}')
            self.assertEqual(res.status_code, 200)
            # self.assertEqual(res.return['result'], "ok")
